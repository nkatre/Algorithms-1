...
static char* TOOL_NAME = TOOL_NAME_DFLT;

void set_verbose_mode(){ VERBOSE = 1; }
void set_pipe_mode(){ PIPE = 1; }
void set_verbose_mode(int i){ VERBOSE = i; }

void set_errors_tool_name(char* tn){ TOOL_NAME = tn;}

int Verbose(char* fmt ...){
	va_list args;
	va_start(args,fmt);
