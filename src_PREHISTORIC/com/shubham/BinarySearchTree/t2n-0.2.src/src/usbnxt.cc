...
int UsbNxt::_fails(const char* msg){
	_usbnxt_error.set("%s - %s", usbmisc_error(), msg);
	return -1;
}
...
int UsbNxt::download(char* fname){
...
bool UsbNxt::_file_exists(char* nme){
	//---------------
...
int UsbNxt::_file_delete(char* nme){
...
*/
	...
	char reply, command, status, fhandler;
